...
{
    public partial class FuncionariosDados : Modelo.ModeloDados
    {
        public FuncionariosDados()
        {
            InitializeComponent();
        }
        ...
        private void Carregar(object sender, EventArgs e)
        {
            try
            {
                ...
                datDataNascimento.MaxDate = System.DateTime.Today;
                ...
                else
                {
                    ...
                    if (dr.Read())
                    {
                        txtNome.Text = dr["NOME_FUNCIONARIO"].ToString();
                        txtCPF.Text = dr["CPF_FUNCIONARIO"].ToString();
                        txtRG.Text = dr["RG_FUNCIONARIO"].ToString();
                        datDataNascimento.Value = Convert.ToDateTime(dr["DATANASC_FUNCIONARIO"].ToString());
                        cbFuncao.Text = dr["FUNCAO_FUNCIONARIO"].ToString();
                        txtSalario.Text = dr["SALARIO_FUNCIONARIO"].ToString();
                        FormatarValor(txtSalario, e);
                        txtTelefone.Text = dr["TELEFONE_FUNCIONARIO"].ToString();
                        txtTelefone2.Text = dr["TELEFONE2_FUNCIONARIO"].ToString();
                        txtEmail.Text = dr["EMAIL_FUNCIONARIO"].ToString();
                        txtCEP.Text = dr["CEP_FUNCIONARIO"].ToString();
                        txtNumero.Text = dr["NUMEROEND_FUNCIONARIO"].ToString();
                        txtComplemento.Text = dr["COMPLEMENTOEND_FUNCIONARIO"].ToString();
                    }
                    if (Operacao == 3)
                    {
                        cbFuncao.DropDownStyle = ComboBoxStyle.Simple;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void Salvar(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("O preenchimento do Salário é obrigatório e deve ser diferente de 0.");
                    txtSalario.Focus();
                    return;
                }
                ...
                BLL.Funcionarios f = new BLL.Funcionarios();
                f.Nome = txtNome.Text;
                f.CPF = txtCPF.Text.Replace("-", "").Replace(",", "").Replace(" ", "");
                f.RG = txtRG.Text.Replace("-", "").Replace(",", "").Replace(" ", "");
                f.DataNasc = datDataNascimento.Value;
                f.Funcao = cbFuncao.Text;
                f.Salario = txtSalario.Text.Replace(",",".");
                f.Telefone = txtTelefone.Text.Replace("-", "").Replace("(", "").Replace(")", "").Replace(" ", "");
                f.Telefone2 = txtTelefone2.Text.Replace("-", "").Replace("(", "").Replace(")", "").Replace(" ", "");
                f.Email = txtEmail.Text;
                f.CEP = txtCEP.Text.Replace("-", "");
                f.NumeroEndereco = txtNumero.Text;
                f.ComplementoEndereco = txtComplemento.Text;
                //TEL
                //EMAIL
                //END
                switch (Operacao)
                {
                    case 1: //CADASTRO
                        f.Incluir();
                        MessageBox.Show("Funcionário(a) " + txtNome.Text + " cadastrado com sucesso.");
                        this.Hide();
                        break;
                    case 2: //ALTERAÇÃO
                        f.Codigo = Codigo;
                        f.Alterar();
                        MessageBox.Show("Funcionário(a) " + txtNome.Text + " atualizado com sucesso.");
                        this.Hide();
                        break;
                    default:
                        this.Hide();
                ...
                FuncionariosLista li = new FuncionariosLista();
                li.ShowDialog();
